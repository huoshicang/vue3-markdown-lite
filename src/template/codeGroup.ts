...
class CodeGroup extends HTMLElement {
  ...
  private tabClick(): void {
    const langTabs = this.shadowRoot.querySelector('slot[name="languageName"]');
    langTabs.assignedElements().forEach((langTabsEl, index) => {

      langTabsEl.addEventListener('click', (event) => {
        event.stopPropagation();
        langTabs.assignedElements().forEach((el) => el.classList.remove('active'))

        langTabsEl.classList.add('active')
        this.blockClick(index)
      })
    })
  }
   ...
   * block事件
   */
  private blockClick(tabIndex: number): void {
    const codeBlocks = this.shadowRoot.querySelector('slot[name="code"]').assignedElements();
    codeBlocks.forEach((codeBlockEl, index) => {
      if (index === tabIndex) {
        codeBlockEl.classList.remove('none')
        codeBlockEl.classList.add('block')
      } else {
        codeBlockEl.classList.remove('block')
        codeBlockEl.classList.add('none')
      }
    })
  }
  ...
  private copyCode(event: Event): void {
    ...
    if (copyBtn && myTextSlot) {

      const blockElement = myTextSlot.assignedElements().find(el => {
       if ( el.classList.contains('block')) return el
      })

      const codeText = (blockElement as HTMLElement)?.innerText;
